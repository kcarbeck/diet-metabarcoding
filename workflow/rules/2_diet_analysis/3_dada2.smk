...
rule dada2_denoise:
    ...
    output:
        ...
        stats_qza    = f"{workdir_path}/dada2/denoise_stats.qza",
        rep_seqs_qzv = f"{workdir_path}/dada2/rep_seqs.qzv",
        table_qzv    = f"{workdir_path}/dada2/table.qzv",
        stats_qzv    = f"{workdir_path}/dada2/denoise_stats.qzv"
    log:
        f"{workdir_path}/logs/dada2.log"
    conda:
        qiime_env
    threads: 4  # ensure >=1
    shell:
        (
            "set -euo pipefail; "
            "mkdir -p {workdir_path}/dada2 {workdir_path}/logs; "
            ...
            " 2>&1 | tee -a {log}; "
            "echo 'DADA2 step complete. Inspect summaries for table, rep-seqs, and stats in {workdir_path}/dada2/' | mail -s '[diet-metabarcoding] Step complete: DADA2' {config[email]}"
        ) 